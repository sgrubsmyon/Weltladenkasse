...
def main():
    ...
    fhz.index = pd.MultiIndex.from_tuples(list(map(lambda i: ('Fairkauf', i[1])
                                                   if i[0] == 'Fairkauf \nHandelsk.' else i, fhz.index.tolist())), names=fhz.index.names)
    fhz.index = pd.MultiIndex.from_tuples(list(map(lambda i: ('Regenwaldladen', i[1])
                                                   if i[0] == 'Regenwald-laden' else i, fhz.index.tolist())), names=fhz.index.names)
    fhz.index = pd.MultiIndex.from_tuples(list(map(lambda i: ('Sonnenglas GmbH', i[1])
                                                   if i[0] == 'Sonnenglas' else i, fhz.index.tolist())), names=fhz.index.names)
